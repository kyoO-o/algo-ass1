...
def api_click():
    try:
        ...
        click_type = str(data.get("type", "unknown"))
        lat = float(data.get("lat"))
        lon = float(data.get("lon"))
        alg = data.get("alg")  # UI-аас одоогийн алгоритмыг бас дамжуулна (сонголт)

        logger.info(
            f"[magenta]MAP CLICK[/] type={click_type} "
            f"lat={lat:.6f} lon={lon:.6f}"
            + (f" alg={alg}" if alg else "")
        )
