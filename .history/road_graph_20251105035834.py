...
def dfs_all_paths(graph: RoadGraph,
    ...
    """
    Итератив DFS – simple path хайна. Зорилтот зангилаанд ойр хөршүүдийг түрүүлж шалгана.
    """
    paths: List[List[int]] = []
    if start == goal:
        return [[start]]
    ...
    def neighbor_iter(u: int):
        nbrs = graph.adj.get(u, [])
        # goal-руу ойр (евклидийн) хөршүүдийг түрүүлж шалгана → хурдан зам олно
        return iter(sorted(
            nbrs,
    ...
    path: List[int] = [start]
    stack: List[Tuple[int, iter]] = [(start, neighbor_iter(start))]
    visited = set(path)
    expanded = 0
    ...
    while stack:
        ...
        try:
            ...
            if expanded >= max_expanded:
                break  # аюулгүйн таслагч – одоог хүртэл олдсон замуудыг буцаана
